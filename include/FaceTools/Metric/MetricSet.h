...
public:
    ...
    // Returns metric - must exist!
    const MetricValue& metric( int id) const;

    // Returns true iff a metric with the given id exists.
    inline bool hasMetric( int id) const { return _ids.count(id) > 0;}
    ...
    void write( PTree& node, float age) const;
