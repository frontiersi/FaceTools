...
#ifndef FACE_TOOLS_TEXTURE_VISUALISATION_H
#define FACE_TOOLS_TEXTURE_VISUALISATION_H

#include "VisualisationAction.h"

namespace FaceTools {

class FaceTools_EXPORT TextureVisualisation : public VisualisationAction
{ Q_OBJECT
public:
    TextureVisualisation( const std::string& iconfile="", bool isDefaultVis=false);
    virtual ~TextureVisualisation(){}

    virtual bool isAvailable( const FaceModel*) const;
    virtual vtkSmartPointer<vtkActor> makeActor( const FaceModel*);
    virtual bool useFloodLights() const { return true;}
    ...
    virtual const QKeySequence* getKeySequence() const { return NULL;}
    virtual QString getDisplayName() const { return "Texture";}
