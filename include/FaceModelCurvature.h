...
{
public:
    using RPtr = std::shared_ptr<const RFeatures::ObjModelCurvatureMap>;
    using WPtr = std::shared_ptr<RFeatures::ObjModelCurvatureMap>;

    // Returns the curvature map for the given FM or null if not available.
...
private:
    static std::unordered_map<const FM*, RFeatures::ObjModelCurvatureMap::Ptr> _metrics;
